...
impl JointEditor {
    pub fn update(
        ...
        match *editing_state {
            EditingState::Create => self.handle_create_joint(pointer_pos, capsules, ui, response),
    ...
    fn handle_create_joint(&mut self, pointer_pos: Pos2, capsules: &[Capsule], ui: &mut egui::Ui, response: &egui::Response) {
        match self.joint_creation_state {
            ...
            JointCreationState::PlaceJoint => {
                ...
                if response.clicked() {
                    self.create_joint(pointer_pos, capsules, ui);
                }
            }
        }
    }
    ...
    fn create_joint(&mut self, pointer_pos: Pos2, capsules: &[Capsule], ui: &mut egui::Ui) {
